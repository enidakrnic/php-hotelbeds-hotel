...
class Client
{
    ...
    /**
     * @param TypesFacilityGroupsIndexRequest $request
    ...
    public function getFacilityGroups($request)
    {
        try {
            $httpClientResponse = $this->getHttpClient()->get(
                $this->getEndpoint().'/hotel-content-api/'.$this->version.'/types/facilitygroups',
                [
                    'headers' => $request->toHeaders($this->getHeaders()),
                    'query' => $request->toQueries()
                ]
            ...
            return new TypesFacilityGroupsIndexResponse($contents);
        } catch (RequestException $requestException) {
            throw $this->requestExceptionToClientException($requestException);
        }
    }
